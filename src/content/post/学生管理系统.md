...
public class StudentSystem {
    ...
    public static boolean addUsername(String username, User user) {
        boolean flag = false;
        //判断用户名长度是否在3~15之间和用户名是否为字母+数字的组合
        if (isUsernameLength(username) && isUsernameStandardize(username)) {
            ...
            //不满足用户名的输入规范,返回false
            System.out.println("请输入'字母+数字'组合的用户名!");
        }
        return flag;
    }
    ...
    public static String getCaptcha() {
        ...
        for (int i = 0; i < captchaArr.length; i++) {
            ...
            char temp = captchaArr[i];
            captchaArr[i] = captchaArr[index];
            captchaArr[index] = temp;
        }
    ...
    public static void forgetPassword(ArrayList<User> list) {
        ...
        if (isUsernameExist(index)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                User user = list.get(i);
                //判断身份证和手机号是否匹配
                if (user.getIdentity().equals(identityCard) && user.getPhone().equals(phone)) {
                    //如果身份证和手机号匹配,则修改密码
